...
export function generarPDF(
  ...
  // ── PLANILLA DE JUSTIFICACIÓN — SALDOS EXCESIVOS ──────────────────────────
  const conSaldoExcesivo = items.filter((i) =>
    i.alertas.some((a) => a.tipo === 'saldo_excesivo') && i.itemMesPosterior
  );

  if (conSaldoExcesivo.length > 0) {
    doc.addPage();
    addHeader();
    y = 28;
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(30, 41, 59);
    doc.text('Planilla de Justificación — Saldos Excesivos', margin, y);
    y += 5;
    ...
    doc.setTextColor(100, 116, 139);
    doc.text(
      'Para cada efecto con saldo excesivo, el responsable logístico debe completar los campos indicados.',
      margin, y
    );
    y += 8;

    for (const item of conSaldoExcesivo) {
      if (y > 220) { doc.addPage(); addHeader(); y = 28; }

      const post = item.itemMesPosterior!;
      const ratio = post.egresos > 0 ? post.saldoQuePasa / post.egresos : null;
      const esRefrig = /carne|picada|pollo|jamon|jamón|queso|manteca|ricota|leche|panceta|bondiola|cerdo|bife|lomo|nalga|paleta|asado|vacio|vacío|peceto|matambre|cuadril|roast|hamburguesa|chorizo/i.test(item.descripcion);

      // Header bar for this item
      ...
      doc.setFontSize(9);
      doc.setTextColor(30, 41, 59);
      doc.text(`[${item.codigo}] ${item.descripcion}`, margin + 3, y + 7);

      const infoRight = `Saldo: ${fmt(post.saldoQuePasa)} ${item.umd}  |  Egresos/mes: ${fmt(post.egresos)} ${item.umd}${ratio !== null ? `  |  Ratio: ${ratio.toFixed(1)}x` : ''}${esRefrig ? '  |  ⚠ REFRIGERADO' : ''}`;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7.5);
      doc.setTextColor(100, 116, 139);
      doc.text(infoRight, pageW - margin - 3, y + 7, { align: 'right' });
      y += 14;

      // Alert detail
      const alertaSaldo = item.alertas.find((a) => a.tipo === 'saldo_excesivo');
      if (alertaSaldo) {
        doc.setFontSize(7.5);
        doc.setFont('helvetica', 'italic');
        doc.setTextColor(154, 52, 18);
        const alertText = doc.splitTextToSize(alertaSaldo.descripcion, contentW - 4);
        doc.text(alertText, margin + 2, y);
        y += alertText.length * 4 + 3;
      }

      // Questions table
      const preguntas = [
        ['1', '¿Dónde se encuentra almacenado actualmente este efecto?', ''],
        ['2', esRefrig
          ? '¿Se dispone de capacidad frigorífica suficiente? Indicar ubicación y temperatura.'
          : '¿Cuál es el lugar de almacenamiento y su capacidad disponible?', ''],
        ['3', '¿El stock ha sido verificado físicamente? ¿Cuándo fue el último recuento?', ''],
        ['4', esRefrig
          ? '¿Se verificó la cadena de frío y fecha de vencimiento del producto?'
          : '¿Se verificó la condición y vigencia del producto?', ''],
        ['5', '¿A qué se atribuye el saldo excesivo? (sobreabastecimiento / baja demanda / error de imputación / otro)', ''],
        ['6', '¿Se proyecta consumir o redistribuir el stock en el próximo período? Indicar plazo.', ''],
      ];

      autoTable(doc, {
        startY: y,
        head: [['N°', 'Pregunta de auditoría', 'Respuesta / Justificación del responsable']],
        body: preguntas,
        margin: { left: margin, right: margin },
        styles: { fontSize: 7.5, cellPadding: 3, minCellHeight: 10 },
        headStyles: { fillColor: [51, 65, 85], textColor: 255, fontStyle: 'bold', fontSize: 7.5 },
        columnStyles: {
          0: { cellWidth: 8, halign: 'center' },
          1: { cellWidth: 85 },
          2: { cellWidth: contentW - 93 },
        },
        theme: 'grid',
      });
      ...
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(71, 85, 105);
      doc.text('Responsable: ___________________________', margin, y);
      doc.text('Cargo: ___________________________', margin + 80, y);
      doc.text('Firma: ________________', margin + 155, y);
      y += 5;
      doc.text('Fecha: _______________', margin, y);
      y += 10;
